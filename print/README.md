...
The pegs are split expansion pins, all four identical. The pin is 2.9 wide across the split and the board's hole is 2.7: the halves compress on the way in and grip the bore by spring friction. Nothing pops out above the board, so the Pi's covered holes take a peg exactly like the open ones, and there is no barb to break. The split shaft squeezes snug into the saddle socket the same way. The board presses on easily and sits snug; a deliberate straight pull lifts it off.
...
- Pegs: standing as the STL opens, eight of them, four plus four spares. Add a brim.
...
- Press the four pegs into the saddle's sockets, they squeeze in snug. All pegs are identical, any peg fits any hole.
...
- Line the board's mounting holes over the peg tips and press down gently, corner by corner, until the board sits on the seat bars. They keep 3mm of air under the board for the microSD card.
- Pi off: a deliberate straight pull, corner by corner. Saddle off: pull straight up, it unplugs.
- Optional anchor, only if the fit is loose: a zip tie around the arm and the Cudy through the arm edge notches.

Fit:

- Cube grip: tune `clr`. Board grip too weak or too fierce: tune `pin_d`. Peg tight or loose in the saddle: tune `shaft_d`. Saddle pulls out too easily: raise `prong_lean`. Insertion fights: lower `prong_lean` or `prong_d`. Base in the well: tune `base_d`. One variable per reprint.
